                    ...
                    foreach (SyntaxTreeNode child in mynode.Children)
                    {
                            ...
                            && child.ParamDict != null)
                        {
                            foreach (KeyValuePair<string, SyntaxTreeNode> kvp in child.ParamDict)
                            {
                                ...
                                {
                                    var p = this.ConstructArgPolish(kvp.Value.Children[0]);
                                    sa.ArgsDict.Add(kvp.Key, this.Folding(p, mynode));
                                }
                                else
                                {
        ...
        private SceneFunction BackpatchOptimizer(SceneAction saNode, SceneAction parent, bool funcFlag)
        {
            ...
            {    
                ...
                case SActionType.act_dialog:
                    ...
                    SceneAction findPtr = iterPtr;
                    int stepCount = 0;
                    bool closeFlag = false;
                    while (findPtr != null && stepCount <= 20)
                    {
                        switch (findPtr.Type)
                        {
                            case SActionType.act_wait:
                            case SActionType.act_waitani:
                            case SActionType.act_waituser:
                            case SActionType.act_yurimsg:
                            ...
                            case SActionType.act_call:
                                closeFlag = true;
                                break;
                            case SActionType.act_dialog:
                                stepCount = int.MaxValue - 1;
                                break;
                        }
                        findPtr = findPtr.Next;
                        stepCount++;
                    }
                    dialogBuilder += closeFlag ? "#0" : "#1";
