...
namespace Yuri.YuriInterpreter
{
    ...
    public sealed class Interpreter
    {
        ...
        {
            var jointIL = String.Join("____YURI_ENGINE____", this.ILVector);
            MD5 digestor = new MD5CryptoServiceProvider();
            var jointILBytes = System.Text.Encoding.UTF8.GetBytes(jointIL);
            var digestHash = digestor.ComputeHash(jointILBytes);
            string digestHashStr = null;
            for (int i = 0; i < digestHash.Length; i++)
            {
                digestHashStr += digestHash[i].ToString("x2");
            }
            try
            {
                FileStream fs = new FileStream(storeFile, FileMode.Create);
                StreamWriter sw = new StreamWriter(fs);
                sw.WriteLine(">>>YuriAEIL?" + digestHashStr + "?" + this.projectName);
