...
namespace Yuri.PlatformCore.Graphic
{
    ...
    internal sealed class ViewManager
    {
        ...
        /// 重绘整个画面
        /// </summary>
        public void ReDraw(bool semiRedrawMessageLayer = false)
        {
            // 先重绘可视化舞台
        ...
        private void DrawSprite2D(YuriSprite sprite, SpriteDescriptor descriptor, ResourceType rType, int idx)
        {
            ...
            if (rType != ResourceType.Background)
            {
                ...
                // 如果存在Mask图样，就要附加点击判定
                if (sprite.IsMaskTriggerable)
                {
                    sprite.DisplayBinding.MouseDown += sprite.MouseDownHandler;
                    sprite.DisplayBinding.MouseUp += sprite.MouseUpHandler;
                }
            }
            else
            {
        ...
        private void DrawMessageLayer(MessageLayer msglay, MessageLayerDescriptor descriptor)
        {
            ...
            msglay.FontSize = GlobalConfigContext.GAME_FONT_FONTSIZE;
            //msglay.FontName = descriptor.FontName;
            msglay.FontName = GlobalConfigContext.GAME_FONT_NAME;
        ...
        private void DrawButton(SpriteButton sbutton, SpriteButtonDescriptor descriptor)
        {
            ...
            // 这里，改成左键only
            buttonImage.MouseLeftButtonDown += sbutton.MouseOnHandler;
            buttonImage.MouseEnter += sbutton.MouseEnterHandler;
            buttonImage.MouseLeave += sbutton.MouseLeaveHandler;
            buttonImage.MouseLeftButtonUp += sbutton.MouseUpHandler;
            if (ViewManager.Is3DStage)
            {
        ...
        private void DrawBranchButton(BranchButton bbutton, BranchButtonDescriptor descriptor)
        {
            ...
            if (GlobalConfigContext.GAME_BRANCH_FONTNAME == "default")
            {
                buttonTextView.FontFamily = new FontFamily(new Uri("pack://application:,,,/"), "Resources/#Source Han Serif CN SemiBold");
            }
            else
            {
        ...
        public static void RenderFrameworkElementToJPEG(FrameworkElement ui, string filename)
        {
            try
            {
                FileStream ms = new FileStream(filename, FileMode.Create, FileAccess.Write);
                RenderTargetBitmap bmp = new RenderTargetBitmap((int)(ui.ActualWidth * (24.0 / 96)), (int)(ui.ActualHeight * (24.0 / 96)), 24d, 24d, PixelFormats.Pbgra32);
                bmp.Render(ui);
