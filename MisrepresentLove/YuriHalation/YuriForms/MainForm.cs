...
namespace Yuri.YuriHalation.YuriForms
{
    public partial class MainForm : Form
    {
        ...
        private void codeListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // 可选择性
            var itemStr = this.codeListBox.SelectedItem.ToString();
            ...
            if (itemStr.Trim().Substring(1) == "循环")
            {
                ...
                for (var i = this.codeListBox.SelectedIndex + 1; i < allAct.Count; i++)
                {
                    if (act.indent == allAct[i].indent && allAct[i].nodeType == ActionPackageType.act_endfor)
                    {
                        for (var j = this.codeListBox.SelectedIndex; j <= i; j++)
                        {
        ...
        private void projTreeView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            this.codeGroupBox.Text = String.Format("脚本 [{0}] (Line: {1})", Halation.currentScriptName, Halation.currentCodePackage.GetAction().Count);
            this.actionGroupBox.Enabled = false;
        }
        ...
        private void codeListBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (e.Index > -1)
            {
                var itemFull = listBox.Items[e.Index].ToString();
                var trimItem = itemFull.Trim().Split(' ')[0];
                Brush FontBrush;
        ...
        {
            var formDialog = new DialogForm("显示对话", false);
        ...
        {
            var mf = new MusicForm("插入音乐", 0, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var mf = new MusicForm("插入音乐", 1, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var mf = new MusicForm("插入音乐", 2, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var sf = new SwitchesForm("开关操作", false);
            sf.ShowDialog(this);
        }
        ...
        {
            var df = new DialogForm("代码片段", false);
            df.ShowDialog(this);
        }
        ...
        {
            var dvf = new DeleteViewForm(0, false);
            dvf.ShowDialog(this);
        }
        ...
        {
            var fcf = new FunctionCallForm(false);
            fcf.ShowDialog(this);
        }
        ...
        private void codeListBox_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {
                this.codeListBox.SelectedIndices.Clear();
                var posindex = this.codeListBox.IndexFromPoint(new Point(e.X, e.Y));
        ...
        {
            var mf = new MusicForm("音乐管理", 0, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var mf = new MusicForm("音乐管理", 1, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var mf = new MusicForm("音乐管理", 2, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var mf = new MusicForm("音乐管理", 3, false);
            mf.ShowDialog(this);
        }
        ...
        {
            var prf = new PicResourceForm("图像资源管理器", 2);
            prf.ShowDialog(this);
        }
        ...
        {
            var prf = new PicResourceForm("图像资源管理器", 1);
            prf.ShowDialog(this);
        }
        ...
        {
            var prf = new PicResourceForm("图像资源管理器", 0);
            prf.ShowDialog(this);
        }
        ...
        {
            var sf = new SwitchesForm("开关管理器", false);
            sf.ShowDialog(this);
        }
        ...
        private void 生成并运行ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                this.core.DashParse();
                var df = new DebugForm(Halation.projectFolder);
                df.ShowDialog(this);
            }
            catch (Exception) { }
        }
        ...
        {
            var upf = new UserPackerForm();
            upf.ShowDialog(this);
        }
        #endregion

        private void codeListBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Space)
            {
                this.toolStripMenuItem5_Click(null, null);
            }
        }
    }
}
